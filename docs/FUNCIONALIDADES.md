...
- **Roles**: `admin_geral`/`admin` (empresa) → `vendedor_master` → `vendedor` (ponta); legado sem role com `vendedor_pai_id` = vendedor, senão admin. `EquipeView.vue` (rota `/equipe`, menu 👥) cria vendedor/Master, vincula conta existente, edita `%`/`ativo` e promove a admin. O campo de comissão do **Master** é oculto (ele recebe o override das faixas). Os limites de desconto mostram o **valor herdado** (padrão da empresa) e permitem **override** por vendedor. A edição usa **`equipe_salvar`** com **snapshot completo** (o front sempre envia os 5 campos: comissão, ativo e descontos), evitando que um payload parcial zere campos (no Xano, `bool`/`decimal` ausente viram `false`/`0`, não `null`).
- **Inativo (efetivo)**: desativar um usuário **ou um "pai"** da hierarquia bloqueia toda a árvore. `f_ativo_efetivo` é aplicado no `auth/login`, no Google `oauth/google/continue`, no `auth/me` (o front desloga com "Conta inativa") e no início dos endpoints de escrita (orçamento, pagamento, equipe, comissões, faixas). A Equipe mostra **"Inativo pelo pai"** quando o membro está ativo mas tem ancestral inativo; reativar o pai restaura.
...
- **Limites**: a **empresa (admin) define o padrão** (`User.desconto_livre_perc`/`desconto_max_perc`); **ponta e Master herdam** quando não têm valor próprio (`null`/`0` = herda) e podem ter **override** (em `/equipe`). O **admin não é limitado**. Sem nada cadastrado → **0** (bloqueia qualquer desconto). O `%` incide sobre a **venda bruta** (`venda_bruta_tot`); filhos **não alteram margem**.
...
- **A.2 (Master + faixas)**: com `Faixa_Comissao` da **empresa** (admin topo da cadeia) cadastrada e um `vendedor_master` na cadeia, calcula pelo **markup efetivo** da orça e lança **2 registros**: ponta (`%` do cadastro, base = **venda `vnd_tot`**) + `override` do Master (`total_faixa − ponta`). **Master vendendo** → **1 registro** com o **total da faixa** (empresa = `master.vendedor_pai_id`). A tela `/faixas` (`FaixasComissaoView`) é um **formulário com labels** (Markup de/até %, Comissão total %, Ordem, Ativo), com **"Sugerir faixas padrão"** (`50–69→7`, `70–89→8`, `90–100→10`).
...
`src/utils/condicoesPagamento.ts` + `src/utils/taxasBanco.ts` (+ `OrcamentosView.vue`), alimentadas por `catalogo.taxasPorCanal(canal)` (`GET /taxas_banco`, cache `orca_taxas_banco_cache_{userId}`):
...
- **Canal de cartão** (`canalCartao`): **Link de pagamento** (`cartao_link`), **Cartão pelo celular** (`cartao_celular`) e **Maquininha (POS)** (`cartao_pos`) — cada canal usa a sua tabela de taxas (`filtrarPorCanal`: empresa específico → empresa genérico → global específico → global genérico). O texto passa a `Cartão de Crédito — Link (3x de R$ X): total de R$ Y.`. Ao abrir sem canal salvo, `aplicarCanalPadrao()` escolhe o **primeiro canal que tiver taxas** (fallback Link).
...
- **Persistência do estado do seletor**: além do texto (`condicoes_pagamento`), `Orca.condicoes_pagamento_params` (JSON) guarda `{ metodos, mesclar, trazerTodasParcelas, descontoPixPercentual, provedorId, provedor, parcelas, canal, repassarTaxas, aba }`, salvo via `orcamento_recalcular`. Ao reabrir o orçamento, `restaurarCondicoesParams()` (em `sincronizarSimulacao` + watch de `catalogo.taxasBanco`) reaplica checkboxes/desconto/instituição/parcelas/canal/repasse; instituição e parcela só voltam se ainda existirem nas taxas atuais. `f_DuplicaOrcamento` copia o campo. `SimulacaoModal` usa o mesmo módulo com defaults.

## Taxas de cartão por empresa + canal (Frente 10)

As taxas **não são globais**: cada **empresa (admin)** tem as suas, e os filhos **herdam** (`f_empresa_id` sobe a cadeia `vendedor_pai_id`). A tabela global (`user_id = null`) é o **default/fallback**.

- **Canais**: `cartao_link` / `cartao_celular` / `cartao_pos`; `canal = null` = **genérico (todos)**. As linhas antigas ficaram `user_id = null` + `canal = null` (comportamento preservado).
- **Backend**: `Taxa_Banco` com `user_id`/`canal`/`origem`/`atualizado_em`; `Provedor` com `url_taxas`/`metodo`/`canal_default`/`seletor`/`ultima_coleta`/`ativo` (preparação p/ scraping). Endpoints: `GET /taxas_banco` (empresa + global, por canal), `GET /taxas_banco_gerenciar` (admin; `admin_geral` via `user_id`), `POST /taxa_banco_salvar`/`taxa_banco_excluir`/`provedor_salvar`. Admin = role `admin`/`admin_geral` **ou** legado sem role e sem pai; escrita reforça `f_ativo_efetivo`.
- **Tela `/taxas`** (`MinhasTaxasView.vue`, menu "Minhas taxas"): abas dos **3 canais** + **"Genérico (todos)"** (só aparece se houver taxa genérica), CRUD provedor × parcelas × taxa, cadastro de provedores; `admin_geral` escolhe a empresa. Exibe também as **taxas globais** (`taxas_globais` do `GET /taxas_banco_gerenciar`) em modo **somente leitura** com selo "padrão" — o fallback aplicado quando a empresa não tem taxa própria no canal — e o botão **"Importar taxas padrão"** (copia as globais do canal para a empresa, tornando-as editáveis e com precedência). Ao salvar/importar, limpa o cache e chama `catalogo.recarregarTaxas()`.
- **Cache**: `orca_taxas_banco_cache_{userId}`; `catalogo.taxasPorCanal(canal)` + `filtrarPorCanal()` (`utils/taxasBanco.ts`).
- **Scraping (futuro)**: usuário parametriza provedores; coleta por **APIs oficiais** + import de tabela padrão (HTML só onde público/estável; Xano não renderiza JS). Valores entram como sugestão (`origem='scrape'`), sem sobrescrever o manual.
