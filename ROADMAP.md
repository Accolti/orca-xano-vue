...
- **Auto-taxas/scraping — infra feita (2026-09)**: agendador **GitHub Actions** (`.github/workflows/coleta-taxas.yml`, cron dias 1 e 16 às 06:00 UTC + manual) rodando o Node `scripts/coleta-taxas/` → endpoints Xano `taxas_coleta_provedores`/`taxas_coleta_importar` (públicos + token `coleta_secret`), grava global `origem='scrape'` sem tocar no manual, atualiza `Provedor.ultima_coleta` e loga em `Taxa_Coleta_Log`. **Pendente**: definir as **fontes** por banco (`url_taxas`/adapter); APIs oficiais (Mercado Pago/Asaas/Pagar.me) + import de tabela padrão; scraping HTML só onde público/estável (páginas JS exigem Playwright, pois o Xano não renderiza JS). Atenção a **ToS**.
