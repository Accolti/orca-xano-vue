...
- **Limites de desconto por usuário** (não herdam; sem cadastro → 0, bloqueia) com aprovação do pai: `Orca.desconto_aprovado`/`desconto_status`, banners por `podeVerPendencia`, fila "Pendentes de aprovação" e **`ConfigComissoesBanner`** (Home/Orçamento/Comissões/`/faixas`) sinalizando faixas/limites não cadastrados
