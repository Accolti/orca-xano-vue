...
- **Limites de desconto por usuário** (não herdam; sem cadastro → 0, bloqueia) com aprovação do pai: `Orca.desconto_aprovado`/`desconto_status`, banners por `podeVerPendencia`, fila "Pendentes de aprovação", **`ConfigComissoesBanner`** (Home/Orçamento/Comissões/`/faixas`) sinalizando faixas/limites não cadastrados, **label de limites + olho** (oculto) no Desconto e no Desconto Pix, e **desconto Pix na mesma política** (`perc = max(desconto, pix)`). Usuário **inativo não loga** (backend + front).
