...
- **Permissões visuais**: filhos não veem custo/lucro/margem/impostos/Frete B2B/Custo Kapazi; herdam a config fiscal/empresa do topo em runtime (`f_perfil_efetivo`), inclusive o cabeçalho da Orca (`regime_id`/`uf_origem`/`uf_destino`); filhos não guardam `organizacao_id`/`uf`/`regime_id`
- **Limites de desconto** (empresa = **padrão**; ponta/Master **herdam** e podem ter **override**; admin livre; sem nada → 0, bloqueia) com aprovação do pai: `Orca.desconto_aprovado`/`desconto_status`, banners por `podeVerPendencia`, fila "Pendentes de aprovação", **`ConfigComissoesBanner`** (Home/Orçamento/Comissões/`/faixas`) sinalizando faixas/limites (efetivo por vendedor), **label de limites + olho** (oculto) no Desconto e no Desconto Pix, e **desconto Pix na mesma política** (`perc = max(desconto, pix)`). Usuário **inativo não loga** — e é **efetivo**: desativar um "pai" bloqueia a árvore toda (login, `auth/me` e reforço nos endpoints de escrita).
...
- **Feito**: tabela `Comissao` (append-only, `user_id`/`orca_id`/`percentual`/`lucro_real_base`/`base_valor`/`tipo` (vendedor|override)/`valor`/`status` calculada|paga/`data_pagamento`); lançamento em `pagamento_baixa` quando pedido `eh_pedido` do vendedor-filho fica 100% pago (lucro real com `Desconto_Kapazi_Log`/frete efetivo, idempotente); `GET /comissoes` (escopo por **árvore/empresa**: admin/vendedor_master veem descendentes + eles; vendedor só as dele; admin_geral tudo; com `PeriodoBar` e totais calculada/paga); `POST /comissao_pagar` (empresa admin ancestral ou admin_geral); `ComissoesView.vue` (rota `/comissoes`, menu 💸) com botão "Marcar paga".
- **A.2 (Master/faixas por markup)**: role `vendedor_master` (nível entre admin e ponta); tabela `Faixa_Comissao` por empresa (`faixa_min/max`, `comissao_total_perc`); engine dinâmica: quando o pedido do ponta fica 100% pago e a empresa tem faixas, calcula pelo **markup efetivo** e lança **2 comissões** (ponta com o % cadastrado + Master override = total da faixa − ponta), base = **venda (`vnd_tot`)**; **Master vendendo** → **1 comissão** com o **total da faixa** (empresa = `master.vendedor_pai_id`); sem faixas → fallback Fase A fixo. Endpoints `faixas_comissao` (GET) e `faixa_comissao_salvar` (POST). Front: `EquipeView` cria Master (admin) e vendedores (admin **ou** master — o master só cria `vendedor`, e o campo de comissão do Master é oculto); `FaixasComissaoView` (rota `/faixas`) é um **formulário com labels** + "Sugerir faixas padrão"; `ComissoesView` mostra Base/tipo; projeção "sua comissão nesta venda" no orçamento (vendedor/Master). **Escopo/pagamento por empresa**: `GET /comissoes` mostra a árvore (descendentes) para `admin`/`vendedor_master` e tudo para `admin_geral`; `POST /comissao_pagar` é permitido à **empresa admin ancestral** do dono ou ao `admin_geral` (o `vendedor_master` não paga).
...
Status: **taxas por empresa + canal implementadas (2026-09)** — cadastro manual em `/taxas`. Pendente: preferências de pagamento, auto-taxas e Asaas. Origem: revisão de "novos usuários" + roadmap Asaas (planos Básico/afiliados, boleto/NF via Asaas).

### Decisões do piloto
- **Herdar o existente por padrão**: enquanto uma conta não cadastra as próprias taxas/condições, usa a **tabela global atual** (`Taxa_Banco`, `user_id = null`) — sem quebra p/ contas atuais.
- **Taxas por empresa (admin)**; `vendedor_master`/`vendedor` **herdam** do topo (via `f_empresa_id`).
- **Cadastro manual a priori**; ideia futura: "inteligência" buscar taxas padrão de adquirentes e popular automaticamente.
- **Distinção de canal de cobrança**: **link de pagamento** (`cartao_link`), **celular** (`cartao_celular`) e **maquininha/POS** (`cartao_pos`); `canal = null` = genérico (vale para todos). Pix/boleto como canal fica para depois.

### Feito ✅ (2026-09)
- `Taxa_Banco` ganhou **`user_id`** (nulo = global/default), **`canal`**, **`origem`** (`manual`|`api`|`scrape`) e `atualizado_em`; `Provedor` ganhou `url_taxas`/`metodo`/`canal_default`/`seletor`/`ultima_coleta`/`ativo` (preparação p/ scraping).
- Função **`f_empresa_id`** (topo da cadeia `vendedor_pai_id`).
- `GET /taxas_banco` resolve **empresa + global** por canal (o front aplica a precedência em `filtrarPorCanal`); `GET /taxas_banco_gerenciar` + `POST /taxa_banco_salvar`/`taxa_banco_excluir`/`provedor_salvar` (admin; `admin_geral` escolhe a conta; reforço `f_ativo_efetivo`).
- Front: seletor de **canal** na aba Cartão do orçamento (persistido em `condicoes_pagamento_params`); texto `Cartão de Crédito — Link (3x de ...)`; cache por usuário (`orca_taxas_banco_cache_{userId}`).
- Tela **`/taxas`** (`MinhasTaxasView.vue`, menu "Minhas taxas"): abas Genérico + 3 canais, CRUD provedor × parcelas × taxa + cadastro de provedores.

### Pendente
- **`Preferencia_Pagamento_User`** (por conta): métodos ativos (Pix/Boleto/Cartão/Link), desconto Pix padrão, máx. parcelas sem juros **por canal**, repasse de taxa, prazos fixos. Forma de receber é **informativa** no piloto (chave Pix/dados da conta p/ referência) — sem emissão.

### Fases futuras (pós-piloto)
- **Auto-taxas/scraping — Nubank ativo (2026-09)**: agendador **GitHub Actions** (`.github/workflows/coleta-taxas.yml`, cron dias 1 e 16 às 06:00 UTC + `workflow_dispatch` com `dry_run`) rodando o Node `scripts/coleta-taxas/` → endpoints Xano `taxas_coleta_provedores`/`taxas_coleta_importar` (públicos + token `coleta_secret`), grava global `origem='scrape'` sem tocar no manual, atualiza `Provedor.ultima_coleta` e loga em `Taxa_Coleta_Log`. **Adapter Nubank sem browser**: lê o `__NEXT_DATA__` da calculadora (`Link de pagamento` → `cartao_link`, `Tap to Pay` → `cartao_celular`, crédito 1x–12x). O import **só regrava quando a taxa muda** (`alterou`); nesse caso o cron chama `taxas_coleta_finalizar` (`versao_taxas_banco +1` + `Configuracoes.taxas_atualizado_em`) e o app exibe o **banner** `TaxasAtualizadasBanner` (admins/empresas) com "Atualizar agora". **Pendente**: adapters dos outros bancos (APIs oficiais Mercado Pago/Asaas/Pagar.me + tabela padrão); páginas 100% JS exigem Playwright (o Xano não renderiza JS). Atenção a **ToS**.
