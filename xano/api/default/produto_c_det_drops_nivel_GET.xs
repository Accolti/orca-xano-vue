...
query produto_C_Det_drops_nivel verb=GET {
  api_group = "Default"
  auth = "User"
  ...
  stack {
    ...
    db.query Produto {
      ...
      where = $db.Produto.detalhe_id > 0 && $db.Material.id ==? $input.material_id && $db.Produto.nivel_id > 0
      eval = {
  ...
  response = $Produto_1
  guid = "sjEegaEhC5PPJIkgq_LlSybksI8"
}