...
query produto_C_Det_drops_linha verb=GET {
  api_group = "Default"
  auth = "User"
  ...
  stack {
    ...
    db.query Produto {
      ...
      where = $db.Produto.detalhe_id > 0 && $db.Material.id ==? $input.material_id && $db.Produto.linha_id > 0
      eval = {
      ...
      return = {
        type : "aggregate"
        group: {
          id         : $db.Produto.linha_id
          nome       : $db.nm_Linha
          material_id: $db.material_id
        }
      }
  ...
  response = $Produto_1
  guid = "CaI9O46h8NVs85OcTT1AF4_IG-M"
}