...
- **`db.edit` grava TODOS os campos do `data` — inclusive os enviados como `null`** (apaga o valor). Endpoints de edição parcial (ex.: `equipe_editar`) **devem** envolver cada campo em `conditional { if ($input.X != null) { db.edit ... } }`; caso contrário, um payload parcial (ex.: alternar `ativo` sem mandar `percentual_comissao`) **zera** os demais. No front, mandar `0` (não `null`) quando a intenção é gravar 0/herdar.
