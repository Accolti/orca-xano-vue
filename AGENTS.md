...
- **Taxas e formas de receber são GLOBAIS hoje**: `Taxa_Banco` (provedor × parcelas × `cc_taxa`) sem `user_id` e `Forma_Pagamento` (ids fixos 1-4); não há distinção de **canal** (link/POS/celular/Pix/boleto) nem tela de cadastro por conta. Modelo "por conta" + Asaas está planejado na **Frente 10 do `ROADMAP.md`**.
...
- **`financeiro`**: pedidos convertidos (`eh_pedido`) por `created_at` com `custo_kapazi` (Σ `item.vlr_cst_nota_unit`×qtd), `perc_desconto` (log mais recente senão `ControlePedido.desconto_kapazi_perc`), `desconto_kapazi`, `frete_efetivo` (`ControlePedido.freteB2BReal` senão `Orca.frtB2B`), **`impostos`** (`Orca.valor_difal_tot − Orca.vlr_credito_icms_tot`), `venda`, `lucro_real = luc_tot + desconto_kapazi + (frtB2B − frete_efetivo)`, `margem_real = lucro_real/vnd_tot × 100` + totais. **A conta fecha visualmente**: `Venda − Custo Kapazi + Desconto − Frete − Impostos = Lucro Real` (o `impostos` era o termo "invisível" que causava a divergência).
